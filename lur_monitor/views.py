...
def todo(req):
    project = Project.objects.all()
    return render(req,"todo.html",{'project':project})
...
def schedules(req):
    ...
    return render(req, "Schedule.html", {'form':form})
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                form.save()
                return redirect("/dashboard/week/me")
            except:
                pass
    else:
        form = contactForm()
    return render(req,"Helpdesk.html",{'form':form})
    ...
    if request.method == "POST":
        ...
        try:
            if user.is_staff:
                ...
                return redirect('dash_week _me')
            else:
                msg = "Invalid Credentials"
        except:
             msg = "login successfully"
             return redirect('dash_week_me')
    dic = {'msg': msg}
